...
public class ItemService {
    ...
    // FIXED: Corrected the parameter usage
    public ExchangeRequest createExchangeRequest(Long requesterItemId, Long requestedItemId) {
        // If requesterItemId is null, it might be a donation request
        if (requestedItemId == null) {
            throw new RuntimeException("Requested item ID cannot be null");
        }

        Item requestedItem = itemRepository.findById(requestedItemId)
                .orElseThrow(() -> new RuntimeException("Requested item not found"));

        // If requester is offering an item in exchange
        Item requesterItem = null;
        if (requesterItemId != null) {
            requesterItem = itemRepository.findById(requesterItemId)
        ...
        // If exchange (not donation), set offered item info
        if (requesterItem != null) {
            request.setOfferedItemId(requesterItemId);
            request.setOfferedItemTitle(requesterItem.getTitle());
            request.setOfferedItemDescription(requesterItem.getDescription());
        }
    ...
    public ExchangeRequest acceptExchangeRequest(Long requestId) {
        ...
        if (request.getOfferedItemId() != null) {
            Item requesterItem = itemRepository.findById(request.getOfferedItemId()).get();
            requesterItem.setStatus("exchanged");
            itemRepository.save(requesterItem);
        }

        Item requestedItem = itemRepository.findById(request.getRequestedItemId()).get();
        requestedItem.setStatus("exchanged");
